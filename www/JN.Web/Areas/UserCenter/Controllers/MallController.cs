...
namespace JN.Web.Areas.UserCenter.Controllers
{
    public class MallController : BaseController
    {
        ...
        public MallController(ISysDBTool SysDBTool, 
        ...
        {
            ActMessage = "所有商品";
            var list = ShopProductService.List(x => x.IsSales).WhereDynamic(FormatQueryString(HttpUtility.ParseQueryString(Request.Url.Query))).OrderByDescending(x => x.ID).ToList();
        ...
        public ActionResult Pay(FormCollection form)
        {
            ...
            if (product != null)
            {
                if (Umodel.Wallet2005 >= product.RealPrice * BuyNumber.ToInt())
                {
                    using (System.Transactions.TransactionScope ts = new System.Transactions.TransactionScope())
                    {
                        lock (obj)
                        {
                            ...
                            Wallets.changeWallet(Umodel.ID, 0 - order.TotalPrice, 2005, "商城消费");
                            ts.Complete();

                            ViewBag.mFormUrl = "/UserCenter/Mall/Order";
