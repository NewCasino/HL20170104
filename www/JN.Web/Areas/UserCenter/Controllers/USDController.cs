    ...
    public class USDController : BaseController
    {
        ...
        public ActionResult Buy(FormCollection form)
        {
            ...
            try
            {
                ...
                //decimal maxbuynumber = puton.ActualPutAmount - puton.DealAmount;  //可交易最大LTC币
                //decimal buyamount = (decimal)TypeConverter.ObjectToFloat(buynumber);

                //首次只能购买等额ＥＰ单
                if (USDPurchaseService.List(x => x.UID == Umodel.ID && x.Status >=0).Count() == 0)
                {
                    if (puton.PutMoney != Umodel.Investment) throw new CustomException("首次只能购买与注册等级相同的EP卖单，您当前的注册等级为：" + Umodel.Investment.ToString("F0"));
                }
                if (USDPurchaseService.List(x => x.UID == Umodel.ID && x.Status >= 0).Count() == 1)
                {
                    if (MvcCore.Unity.Get<ICFBSubscribeService>().List(x => x.UID == Umodel.ID && x.CanBeUsed == 0).Count() == 1)
                        throw new CustomException("请在解冻股权后再购买EP");
                }
                if (USDSeekService.List(x => x.UID == Umodel.ID && x.Status >= 0).Count() == 1)
                {
                    if (MvcCore.Unity.Get<ICFBSubscribeService>().List(x => x.UID == Umodel.ID && x.CanBeUsed == 0).Count() == 1)
                        throw new CustomException("请在解冻股权后再购买EP");
                }
                ...
                {
                    SMSHelper.WebChineseMSM(onUser.Mobile, "账户：" + Umodel.UserName + "购买了你的EP，请留意收款");
                    SMSHelper.WebChineseMSM(Umodel.Mobile, "你购买了" + onUser.UserName + "的EP，请在" + (cacheSysParam.SingleAndInit(x => x.ID == 2202).Value.ToInt() / 60) + "小时内完成付款");
                }
        ...
        public ActionResult ApplyPutOn(FormCollection form)
        {
            ...
            try
            {
                ...
                string putonmoney = form["putonnumber"];
                ...
                if (putonmoney.ToDecimal() <= 0) throw new CustomException("交易金额不正确");
                ...
                string description = "卖出EP(从交易大厅)";//（手续费：" + poundage + "）";
                if (Umodel.Wallet2002 < changeMoney) throw new CustomException("您的帐户余额不足");
                ...
                if (Seek != null)
                {
                    ...
                    if (putonmoney.ToDecimal() > (Seek.SeekMoney - (Seek.HaveDeal ?? 0))) throw new CustomException("卖出金额不能高于求购金额");
                ...
                lock (this)
                {
                    ...
                    {
                        //写入交易表
                        var newmodel = new Data.USDPutOn { MoneyType = moneytype, PutonNumber = USDPutOn.GetPutOnNumber(), PutMoney = putonmoney.ToDecimal(), Phone = Umodel.Mobile, PayType = paytype, CreateTime = DateTime.Now, DealAmount = 0, Poundage = 0, PutAmount = changeMoney, Status = 1, UID = Umodel.ID, UserName = Umodel.UserName, SeekID = seekid.ToInt() };
                        USDPutOnService.Add(newmodel);
                        SysDBTool.Commit();
                        Wallets.changeWallet(Umodel.ID, 0 - changeMoney, 2002, description);
                ...
                if (seekUser != null && !string.IsNullOrEmpty(seekUser.Mobile))
                {
                    ...
                    SMSHelper.WebChineseMSM(Umodel.Mobile, "账户：" + Umodel.UserName + "您卖出的EP已匹配成功，请留意收款");
                }
            }
            catch (CustomException ex)
            {
        ...
        public ActionResult ApplySeek(FormCollection form)
        {
            ...
            try
            {
                //string seeknumber = form["seeknumber"];
                ...
                string tradePassword = form["tradeinPassword"];
                if (Umodel.Password2 != tradePassword.ToMD5().ToMD5()) throw new CustomException("二级密码不正确");
                if (seekmoney.ToDecimal() <= 0) throw new CustomException("求购金额不正确");
                // if (remark.Trim().Length > 250) throw new CustomException("备注信息不能超过250个字符");
                if (Umodel.IsLock) throw new CustomException("您的帐号受限，无法进行相关操作");
                //首次只能购买等额ＥＰ单
                if (USDPurchaseService.List(x => x.UID == Umodel.ID && x.Status >= 0).Count() == 0)
                {
                    if (seekmoney.ToDecimal() != Umodel.Investment) throw new CustomException("首次只能求购与注册等级相同的EP求购单，您当前的注册等级为：" + Umodel.Investment.ToString("F0"));
                }

                int ckupgrade = 0;
                if (form["ckupgrade"] == "true")
                {
                    if (seekmoney.ToDecimal() < Umodel.Investment) throw new CustomException("升级投资选择额度必须大于本身级别额度");
                    if (USDPurchaseService.List(x => x.UID == Umodel.ID && x.Status >= 0).Count() == 0) throw new CustomException("首次求购无法勾选升级选项");
                        ckupgrade = 1;
                }


                if (USDPurchaseService.List(x => x.UID == Umodel.ID && x.Status >= 0 && x.Status < (int)Data.Enum.USDStatus.Deal).Count() == 1)
                     throw new CustomException("您有正在交易中的订单，请在交易完成后再购买EP");
                if (USDSeekService.List(x => x.UID == Umodel.ID && x.Status >= 0 && x.Status < (int)Data.Enum.USDStatus.Deal).Count() == 1)
                    throw new CustomException("您有进行中的求购订单，请在交易完成后再购买EP，");

                if (MvcCore.Unity.Get<ICFBSubscribeService>().List(x => x.UID == Umodel.ID && x.Status < (int)Data.Enum.SubscribeStatus.AllUsed && x.Origin == "EP").Count() == 1)
                    throw new CustomException("请在解冻股权后再购买EP");
